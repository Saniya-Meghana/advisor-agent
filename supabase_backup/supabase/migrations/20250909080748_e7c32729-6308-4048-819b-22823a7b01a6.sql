...
-- Create regulation_templates table (created_by nullable for system templates)
CREATE TABLE public.regulation_templates (
    ...
    created_by UUID, -- nullable for system templates
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
    updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
-- Insert default regulation templates (system templates with null created_by)
