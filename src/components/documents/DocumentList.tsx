...
import { FileText, Download, Trash2, AlertTriangle, CheckCircle, Clock, Eye } from 'lucide-react';
...
const DocumentList: React.FC<DocumentListProps> = ({ onDocumentChange }) => {
  ...
  const deleteDocument = async (doc: Document) => {
    try {
      ...
      // Delete from database (this will cascade to compliance_reports)
      ...
      toast({
        ...
        description: `${doc.original_name} has been removed`,
      });
    } catch (error: any) {
  ...
  const retryAnalysis = async (doc: Document) => {
    try {
      ...
      toast({
        ...
        description: error.message || 'Failed to queue document for analysis',
        variant: "destructive",
      });
    }
  };
