...
serve(async (req) => {
  ...
  try {
    ...
    const { message, session_id, user_id, reset } = await req.json();
...
function buildContext(
  ...
  if (reports?.length) {
    ...
    for (const report of reports) {
      ...
      if (report.analysis_summary) {
        context += `  Summary: ${report.analysis_summary.slice(0, 200)}...\n`;
      }
      context += `\n`;
    }
  }
  ...
  if (recentMessages?.length) {
    ...
    for (const msg of recentMessages.reverse()) {
      context += `${msg.message_type === "user" ? "User" : "Assistant"}: ${msg.content.slice(0, 150)}${msg.content.length > 150 ? "..." : ""}\n`;
    }
    context += `\n`;
  }
