...
-- Create security definer function to check roles with app_role enum
...
CREATE TABLE IF NOT EXISTS public.regulation_templates (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL,
  description TEXT,
...
-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Everyone can view active templates" ON public.regulation_templates;
DROP POLICY IF EXISTS "Admins can manage templates" ON public.regulation_templates;
...
DROP TRIGGER IF EXISTS update_regulation_templates_updated_at ON public.regulation_templates;
